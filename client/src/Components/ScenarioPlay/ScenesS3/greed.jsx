...
function Buildings() {
  return (
    <>
      <StaticModel path="./building1.glb" scale={[7, 7, 7]} position={[-50, -25, -20]} />
      <StaticModel path="./building1 copy.glb" scale={[8, 8, 8]} position={[-50, -25, -40]} />
      <StaticModel path="./building2.glb" scale={[0.25, 0.25, 0.25]} position={[50, 21, -70]} rotation={[0, Math.PI, 0]} />
    </>
  );
}
...
function Cars() {
  return (
    <>
      <StaticModel path="./cars.glb" scale={[2, 2, 2]} position={[20, -29, -60]} />
    </>
  );
}
...
export default function CityScene() {
  ...
  return (
    <div style={{ width: '100vw', height: '100vh' }}>
      ...
      <Canvas camera={{ position: [-6.62444165291652,  10.392926904921868, 74.6198510372118], fov: 50 }}>
        <ambientLight intensity={0.4} />
        <directionalLight position={[10, 20, 10]} intensity={0.8} />
        {/* Sky background */}
        <Skybox texturePath="./sky.jpg" /> {/* Replace with your sky texture file */}
        {/* Ground as a 3D Model */}
        <StaticModel path="./grass.glb" scale={[2, 2, 2]} position={[0, 0, 0]} />
        <Buildings />
        <Cars />
        {/* Animated Models */}
        <AnimatedModel path="./man.glb" scale={[4, 4, 4]} position={[-40, -25, 5]}  rotation={[0,-Math.PI/3, 0]}/>
        <AnimatedModel path="./woman.glb" scale={[4, 4, 4]} position={[-44, -25, 5]} rotation={[0, Math.PI/3, 0]} />
        <Bird /> {/* Animated bird */}
        <OrbitControls />
        <CameraLogger />
      </Canvas>
