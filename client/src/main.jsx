...
// Create an instance of the QueryClient for React Query
const client = new QueryClient();

// Create the root of the app and render it into the 'root' div in the HTML
ReactDOM.createRoot(document.getElementById("root")).render(
  <React.StrictMode>
    {/* Redux Provider to make the store accessible to all components */}
    <Provider store={store}>
      {/* React Query Provider for handling server-side data */}
      <QueryClientProvider client={client}>
        {/* Main app component */}
        <App />
        {/* React Query Devtools for debugging, initially closed */}
        <ReactQueryDevtools initialIsOpen={false} />
      </QueryClientProvider>
    </Provider>
  </React.StrictMode>
